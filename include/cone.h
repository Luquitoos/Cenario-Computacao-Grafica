...
public:
    ...
    cone(const point3& ap, const vec3& ax, double ang, double h,
        ...
        : apex(ap), axis(unit_vector(ax)), angle(ang), height(h), 
          mat(m), name(obj_name) {}
            ...
            for (int i = 0; i < 2; i++) {
                ...
                if (t >= t_min && t < best_t) {
                    ...
                    if (h_point >= 0 && h_point <= height) {
                        ...
                        best_normal = unit_vector(radial - tan_a * h_point * unit_vector(radial));
                        // Correção: normal apontando para fora
                        best_normal = unit_vector(cp - (1.0 + tan_a * tan_a) * h_point * axis);
